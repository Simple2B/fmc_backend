...
def upgrade():
    ...
    op.alter_column(
        "messages", "author_id", existing_type=sa.VARCHAR(length=36), nullable=True
    )
    op.add_column(
        "students_lessons", sa.Column("review_id", sa.Integer(), nullable=True)
    )
...
def downgrade():
    ...
    op.alter_column(
        "messages", "author_id", existing_type=sa.VARCHAR(length=36), nullable=False
    )
